...
export const ClassDetailPage: React.FC = () => {
      ...
      try {
        ...
        if (familyData) {
          const [status, enrolled, waitlisted] = await Promise.all([
            checkWaitlistStatus(classId, 'class', familyData.id),
            getEnrolledMemberIds(classId, 'class', familyData.id),
            getWaitlistedMemberIds(classId, 'class', familyData.id),
          ]);
  ...
  const eligibleMembersCount = useMemo(() => {
    ...
    const allowParents = classData?.allowParents ?? false;
  ...
  }, [familyData?.members, enrolledMemberIds, waitlistedMemberIds, classData?.allowParents]);
  ...
  const handleAddToCart = () => {
    ...
    if (eligibleMembersCount === 0) {
      setError('No eligible family members to enroll. All members are either already enrolled, on the waitlist, or not eligible for this class.');
      return;
    }
  ...
  const handleJoinWaitlist = () => {
    ...
    if (eligibleMembersCount === 0) {
      setError('No eligible family members to add to the waitlist. All members are either already enrolled, on the waitlist, or not eligible for this class.');
      return;
    }
  ...
  const handleMemberSelection = async (selectedMemberIds: string[]) => {
    ...
    if (isFree) {
      ...
      try {
        ...
        const enrolled = await getEnrolledMemberIds(classId, 'class', familyData.id);
        setEnrolledMemberIds(enrolled);
    ...
    <div className="container-custom py-12">
                    ...
                    <div className="flex items-start">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-amber-700">
                          All family members are already enrolled, on the waitlist, or not eligible for this class.
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      {familyData && (
        <MemberSelectionModal
          ...
          allowParents={classData?.allowParents ?? false}
        />
      )}
